 ...
 */
package org.cagrid.index.service.impl.database.xml.xindice;
...
public class XindiceIndexDatabase {
    static Log logger = LogFactory.getLog(XindiceIndexDatabase.class.getName());
    ...
    public XindiceIndexDatabase(String collectionName) {
    ...
    private void initializeCollection(String collectionName, boolean createDefaultIndex, boolean replaceExisting)
            throws Exception {
        try {
            // if (this.isProfiling) {
            ...
            if (!this.db.findCollection(this.rootURI, this.BASE_COLLECTION_NAME)) {
                synchronized (this.db) {
                    this.db.createCollection(this.rootURI, this.BASE_COLLECTION_NAME);
                }
                this.db.checkpointCollection(this.rootURI + "/" + this.BASE_COLLECTION_NAME);
            }
    ...
    public String addDocument(String collectionURI, String docName, Object doc, boolean asString) throws Exception {
        ...
        try {
            ...
            if (asString) {
                docID = docID + this.db.addDocumentString(collectionURI, (String) doc, docName);
                // if (this.isProfiling) {
                // this.performanceLogger.stop("addData(string) " + docName);
                // }
            } else {
                docID = docID + this.db.addDocumentDOM(collectionURI, (Node) doc, docName);
                // if (this.isProfiling) {
    ...
    public void removeDocument(String collectionURI, String docName) throws Exception {
        try {
            rwLock.writeLock().lock();
    ...
    public List<Element> query(String xpath, String[] namespaces) throws Exception {
        ...
        try {
            ...
            Object[] queryResults = this.db.XPathQuery(URI, xpath, namespaces, XindiceDriver.XINDICE_DOCTYPE_DOM);

            if (queryResults != null) {
                for (int i = 0; i < queryResults.length; i++) {
                    // TODO: what do I need to do here? I think jaxb is fine with Dom Documents
                    // results.add(new MessageElement(((Document) queryResults[i]).getDocumentElement()));
                    results.add(((Document) queryResults[i]).getDocumentElement());
                }
            }
    ...
    private synchronized void addDefaultIndexers(String colURI) throws Exception {
        ...
        if (this.defaultElementIndexPattern != null) {
            this.db.addIndexer(colURI, DEFAULT_ELEMENT_INDEX_NAME, null, this.defaultElementIndexPattern);
            this.indexAdded = true;
        }
        if (this.defaultAttributeIndexPattern != null) {
            this.db.addIndexer(colURI, DEFAULT_ATTRIB_INDEX_NAME, null, this.defaultAttributeIndexPattern);
            this.indexAdded = true;
        }
    }
